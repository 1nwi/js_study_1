...
/*
    시스템은 1~20 사이의 무작위의 정수 2개를 생성하여
    덧셈 문제를 출제해야 한다.
    ...
    시스템은 지속적으로 다른 문제를 출제하여 사용자가 0을 입력할 때 까지 답을 계속 입력받고 검증해줘야 한다.
*/
...
while (true) {
    ...
    if (markNum === 0) {
        ...
        mark = '-';
        if (first === second) {     // 정답이 0일떄 종료되는걸 막기위한 주작
            second--;
        } else if (first < second) {    // 정답이 -가 안나오도록 주작
            var t = first;
            first = second;
            second = t;
        }
    ...
    // 종료 조건
    if (userAnswer === 0) {
        alert(`게임을 종료합니다!\n------------------\n정답횟수: ${okCount}회, 틀린횟수: ${wrongCount}`);
        break;
    }
