...
if (level === 1) {
    alert('난이도를 상을 선택했습니다. 3번만에 맞추시면 됩니다.')
    INIT_COUNT = 3;
} else if (level === 2) {
    alert('난이도를 중을 선택했습니다. 6번만에 맞추시면 됩니다.')
    INIT_COUNT = 6;
} else if (level === 3) {
    alert('난이도를 하을 선택했습니다. 10번만에 맞추시면 됩니다.')
    INIT_COUNT = 10;
} else {
...
while (true) {
    ...
    if (secret === userAnswer) {
        alert(`정답입니다! ${INIT_COUNT - countDown}번 만에 맞추셨습니다.`);
        break;  // 게임 종료 선조건
